enyo.kind({
	...
	finish: function() {
		...
		for (var i=0, c; c=c$[i]; i++) {
			...
			// c.removeClass("page-turn-animation");
		}
	},
	...
	boxTurnControl: function(inControl, inAngle, inTransform, inScale, inOpacity) {
		...
		if(enyo.dom.canAccelerate) {
			var a = inAngle;
			// var temp = inControl.domCssText;
			// var pattern = /rotateY\((\d)+deg\)/;
			// var currentAngle = pattern.exec(temp);
			// if(currentAngle&&currentAngle[1]!=a)
			// 	inControl.addClass("page-turn-animation");
			// else
			// 	inControl.removeClass("page-turn-animation");
